...
export function ApiKeysMutateDrawer({
  ...
  const { data: subscriptionData } = useQuery({
    queryKey: ['user-subscription-self'],
    queryFn: getUserSubscriptionSelf,
  ...
  const form = useForm<ApiKeyFormValues>({
    ...
    defaultValues: getApiKeyFormDefaultValues(defaultUseAutoGroup, isAdmin),
  })
  ...
  useEffect(() => {
    ...
    if (isUpdate && currentRow) {
      ...
      form.reset(
        getApiKeyFormDefaultValues(defaultUseAutoGroup && backendHasAuto, isAdmin)
      )
      setInitializedTarget(target)
    }
  }, [
                ...
                <div className='rounded-md border border-dashed border-muted-foreground/40 bg-muted/30 px-3 py-2 text-xs text-muted-foreground'>
                  {activeSubscriptionSummary ? (
                    <div className='flex flex-col gap-1'>
                      <div>
                        {t('Active subscription')}: #{activeSubscriptionSummary.id}
                      </div>
                      <div>
                        {t('Will expire at')}: {new Date(activeSubscriptionSummary.end_time * 1000).toLocaleString()}
                      </div>
                      <div>
                        {t('Remaining quota')}: {formatRawTokens(Math.max(activeSubscriptionSummary.amount_total - activeSubscriptionSummary.amount_used, 0))} {t('tokens')}
                      </div>
                      {activeSubscriptionSummary.upgrade_group && (
                        <div>
                          {t('Group')}: {activeSubscriptionSummary.upgrade_group}
                        </div>
                      )}
                      <div>
                        {(() => {
                          const list = (activeSubscriptionSummary.allowed_models ?? '')
                            .split(',')
                            .map((s) => s.trim())
                            .filter(Boolean)
                          if (list.length === 0) {
                            return `${t('Allowed Models')}: ${t('All')}`
                          }
                          return `${t('Allowed Models')}: ${list.join(', ')}`
                        })()}
                      </div>
                    </div>
                  ) : (
                    <div>
                      {t('No active subscription. Default: 30-day expiry, fixed initial quota.')}
                    </div>
                  )}
                </div>
              )}
