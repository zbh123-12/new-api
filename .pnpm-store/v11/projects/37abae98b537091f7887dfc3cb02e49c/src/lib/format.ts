...
/**
 * Format a raw quota value as a plain token count with thousands separators.
 *
 * Unlike {@link formatQuota}, this never converts the value into a currency -
 * the unit is the same unit stored in the database (tokens). Intended for
 * surfaces where the operator only cares about how many tokens are granted
 * (e.g. subscription plan quota columns and purchase dialogs).
 */
export function formatRawTokens(quota: number): string {
  if (!Number.isFinite(quota) || quota <= 0) return '0'
  return new Intl.NumberFormat(undefined, { maximumFractionDigits: 0 }).format(quota)
}

/**
 * Compose a token-count label like `500000 tokens`.
 * Returns the localized `Unlimited` string for non-positive values.
 */
export function formatRawTokensWithUnit(
  quota: number,
  unitText: string,
  unlimitedText: string
): string {
  if (!Number.isFinite(quota) || quota <= 0) return unlimitedText
  return `${formatRawTokens(quota)} ${unitText}`
}
